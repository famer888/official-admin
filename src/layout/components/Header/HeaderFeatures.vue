<template>
  <div class="relative flex items-center h-16" ref="headerFeaturesRef">
    ...
    <div class="relative" ref="messageTriggerRef">
      <div
        class="flex flex-col items-center justify-center px-3 py-2 cursor-pointer rounded transition-all relative hover:bg-white/8"
      ...
      >
        ...
        <span
          class="text-xs leading-tight whitespace-nowrap text-[#455980] transition-colors"
          :class="{ 'text-[#3A82F9]': activeFeature === 'message' }"
          >站内消息</span
        >
      </div>
      ...
      >
        <template #item="{ item, index }">
          ...
          <div class="flex-1 min-w-0 overflow-hidden flex items-center">
            <div
              class="text-[15px] font-medium leading-[1.4] whitespace-nowrap overflow-hidden text-ellipsis"
              :class="
                selectedMessageIndex === index || hoveredMessageIndex === index
                  ? 'text-[#3A82F9]'
                  : 'text-[#455980]'
              "
              >{{ item.message }}</div
    ...
    >
      ...
      <span class="text-xs leading-tight whitespace-nowrap text-[#EB445A] transition-colors"
        >USD:{{ state.usdBalance }}</span
      >
    </div>
    <div class="w-px h-10 bg-[#DEE5EE] mx-1"></div>
    <!-- 用户ID -->
    ...
    >
      ...
      <span
        class="text-xs leading-tight whitespace-nowrap text-[#455980] transition-colors"
        :class="{ 'text-[#3A82F9]': activeFeature === 'userId' }"
        >ID:{{ state.userId }}</span
      >
    </div>
    <div class="w-px h-10 bg-[#DEE5EE] mx-1"></div>
    <!-- 常见问题 -->
    <div class="relative" ref="faqTriggerRef">
      ...
      >
        ...
        <span
          class="text-xs leading-tight whitespace-nowrap text-[#455980] transition-colors"
          :class="{ 'text-[#3A82F9]': activeFeature === 'faq' }"
          >常见问题</span
        >
      </div>
    </div>
    ...
    >
      ...
      <span
        class="text-xs leading-tight whitespace-nowrap text-[#455980] transition-colors"
        :class="{ 'text-[#3A82F9]': activeFeature === 'logout' }"
        >登出</span
      >
    </div>
    ...
    >
      <button
        class="w-[180px] h-[47px] bg-[#3A82F9] border-none text-white text-base rounded cursor-pointer outline-none flex items-center justify-center gap-1.5 focus:outline-none shadow-lg"
    ...
    >
      <template #header>
        <button
          class="w-[192px] h-10 bg-[#3A82F9] border-none text-white text-sm rounded cursor-pointer outline-none flex items-center justify-center gap-1.5 focus:outline-none hover:opacity-90"
          @click="goToHelpCenter"
        >
          前往帮助中心
      ...
      <template #item="{ item, index }">
        ...
        <div
          v-if="item.type !== 'hint'"
          class="flex-shrink-0 flex items-center justify-center mr-[17px]"
          :class="item.type === 'avatar' ? 'w-8 h-8' : 'w-7 h-7'"
        >
          <img
        ...
        <div
          :class="
            item.type === 'hint'
              ? 'w-full text-center'
              : 'flex-1 min-w-0 overflow-hidden flex items-center'
          "
        >
          <div
