      ...
      <n-gi v-bind="schema.giProps" v-for="schema in getSchema" :key="schema.field">
        <n-form-item :label="schema.label" :path="schema.field">
          ...
          <template v-else-if="schema.component === 'NCheckbox'">
            <n-checkbox-group v-model:value="formModel[schema.field]" v-bind="schema.componentProps">
              <n-space>
                <n-checkbox v-for="item in schema.componentProps.options" :key="item.value" :value="item.value"
                  :label="item.label" />
              </n-space>
            </n-checkbox-group>
          </template>
          ...
          <template v-else-if="schema.component === 'NUpload'">
            <CustomUpload v-model:value="formModel[schema.field]" v-bind="schema.componentProps" />
          </template>

          <!--NTree-->
          <template v-else-if="schema.component === 'NTree'">
            <n-tree v-model:checked-keys="formModel[schema.field]" v-bind="schema.componentProps" />
          </template>

          <!--NTreeSelect-->
          <template v-else-if="schema.component === 'NTreeSelect'">
            <n-tree-select v-model:value="formModel[schema.field]" v-bind="schema.componentProps" />
          </template>

          <!--NRadioGroup-->
          <template v-else-if="schema.component === 'NRadioGroup'">
            <n-radio-group v-model:value="formModel[schema.field]" v-bind="schema.componentProps">
              <n-space>
          ...
          <!--动态渲染表单组件-->
          <component v-else v-bind="getComponentProps(schema)" :is="schema.component"
            v-model:value="formModel[schema.field]" :class="{ isFull: schema.isFull != false && getProps.isFull }">
            <!-- ① schema.suffix 作为“插槽名”，父组件提供内容： -->
            <template v-if="typeof schema.suffix === 'string' && $slots[schema.suffix]" #suffix>
              <slot :name="schema.suffix" :model="formModel" :field="schema.field" :value="formModel[schema.field]" />
            </template>

            <!-- ② schema.suffix 是“直接内容”（如 '元'）：-->
            <template v-else-if="typeof schema.suffix === 'string'" #suffix>
              {{ schema.suffix }}
            </template>

            <!-- ③ schema.suffix 是函数，返回 VNode：-->
            <template v-else-if="typeof schema.suffix === 'function'" #suffix>
              <component :is="schema.suffix({
                model: formModel,
                field: schema.field,
...
export default defineComponent({
  name: 'BasicForm',
  components: { DownOutlined, UpOutlined, QuestionCircleOutlined, CustomUpload },
  props: {
    ...basicProps,
  },
