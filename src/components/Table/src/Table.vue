...
<script lang="ts" setup>
  ...
  const unsortedDataRef = ref<Recordable<any>[] | null>(null)

  //页码切换
  function updatePage(page) {
    unsortedDataRef.value = null
  ...
  function onSorterChange({ columnKey, order }) {
    const columns = toRaw(unref(getPageColumns)) || []
    const col = columns.find((c) => c.key === columnKey)
    if (!col) return

    const enabled = col.sorter === true || typeof col.sorter === 'function'
    if (!enabled) return

    if (!unsortedDataRef.value) {
      unsortedDataRef.value = [...unref(getDataSourceRef)]
    }

    if (!order) {
      setTableData(unsortedDataRef.value)
      unsortedDataRef.value = null
      return
    }
    ...
    const compare = typeof col.sorter === 'function' ? col.sorter : defaultCompare

    data.sort((a, b) => (order === 'ascend' ? -compare(a, b) : compare(a, b)))
    setTableData(data)
  }
